...
{
    public static Cube1 CurrentCube { get; private set; }
	...
	void Start ()
    {
        ...
        if (CubeNum > 0)
            originPos.x = transform.position.x + transform.localScale.x;
        else
            originPos.x = 0;
    }
    ...
    void Update ()
    {
        ...
        if (this.transform.position.x >= originPos.x + 2.5f)
        {
            Direction = -1.0f;
            if (MoveSpeed > 3.0f)
        ...
        else if (this.transform.position.x <= originPos.x - 2.5f)
        {
            Direction = 1.0f;
            if (MoveSpeed > 3.0f)
                MoveSpeed = MoveSpeed - 0.3f;
